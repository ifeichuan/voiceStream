use crate::audio::convert_chunk_to_pcm16;
use crate::stt::{
    mark_runtime_error, mark_runtime_finished, push_final_transcript,
    update_partial_transcript, SttProvider, PROVIDER_DEEPGRAM,
};
use crate::stt_providers::{emit_status, emit_transcript};
use futures_util::{SinkExt, StreamExt};
use serde_json::Value;
use tauri::AppHandle;
use tokio::sync::mpsc;
use tokio_tungstenite::{
    connect_async,
    tungstenite::{client::IntoClientRequest, Message},
};

const TARGET_SAMPLE_RATE: u32 = 16_000;
const TARGET_CHANNELS: u16 = 1;

enum Cmd {
    Audio { pcm: Vec<i16>, sample_rate: u32, channels: u16 },
    Finish,
}

pub struct DeepgramSttProvider {
    sender: mpsc::UnboundedSender<Cmd>,
}

impl SttProvider for DeepgramSttProvider {
    fn push_chunk(&self, pcm: Vec<i16>, sample_rate: u32, channels: u16) -> Result<(), String> {
        self.sender
            .send(Cmd::Audio { pcm, sample_rate, channels })
            .map_err(|_| "Deepgram session closed".to_string())
    }

    fn finish(&self) -> Result<(), String> {
        self.sender
            .send(Cmd::Finish)
            .map_err(|_| "Deepgram session closed".to_string())
    }
}

impl DeepgramSttProvider {
    pub fn new(app: AppHandle, api_key: String, endpoint: String, model: String, language: String) -> Self {
        let (sender, receiver) = mpsc::unbounded_channel();

        tauri::async_runtime::spawn(async move {
            if let Err(error) = run_session(app.clone(), api_key, endpoint, model, language, receiver).await {
                mark_runtime_error(&error);
                emit_status(&app, PROVIDER_DEEPGRAM, &format!("error: {}", error));
            }
        });

        Self { sender }
    }
}

/// Build the Deepgram WebSocket URL with query parameters.
fn build_url(endpoint: &str, model: &str, language: &str) -> String {
    let mut url = endpoint.trim_end_matches('/').to_string();

    let mut params = vec![
        format!("encoding=linear16"),
        format!("sample_rate={}", TARGET_SAMPLE_RATE),
        format!("channels={}", TARGET_CHANNELS),
        format!("punctuate=true"),
        format!("interim_results=true"),
    ];

    if !model.is_empty() {
        params.push(format!("model={}", model));
    }
    if !language.is_empty() {
        params.push(format!("language={}", language));
    }

    let separator = if url.contains('?') { "&" } else { "?" };
    url.push_str(separator);
    url.push_str(&params.join("&"));
    url
}

async fn run_session(
    app: AppHandle,
    api_key: String,
    endpoint: String,
    model: String,
    language: String,
    mut receiver: mpsc::UnboundedReceiver<Cmd>,
) -> Result<(), String> {
    emit_status(&app, PROVIDER_DEEPGRAM, "connecting");

    let url = build_url(&endpoint, &model, &language);
    let mut request = url
        .into_client_request()
        .map_err(|e| format!("invalid Deepgram endpoint: {}", e))?;

    request.headers_mut().insert(
        "Authorization",
        format!("Token {}", api_key)
            .parse()
            .map_err(|e| format!("invalid auth header: {}", e))?,
    );

    let (socket, _) = connect_async(request)
        .await
        .map_err(|e| format!("Deepgram connect failed: {}", e))?;
    let (mut writer, mut reader) = socket.split();

    emit_status(&app, PROVIDER_DEEPGRAM, "listening");
    let mut finish_sent = false;

    loop {
        tokio::select! {
            Some(cmd) = receiver.recv(), if !finish_sent => {
                match cmd {
                    Cmd::Audio { pcm, sample_rate, channels } => {
                        let payload = convert_chunk_to_pcm16(
                            &pcm, sample_rate, channels,
                            TARGET_SAMPLE_RATE, TARGET_CHANNELS,
                        );
                        if !payload.is_empty() {
                            writer.send(Message::Binary(payload.into()))
                                .await
                                .map_err(|e| format!("audio send failed: {}", e))?;
                        }
                    }
                    Cmd::Finish => {
                        // Deepgram: send empty byte message or close_stream JSON
                        let close_msg = serde_json::json!({"type": "CloseStream"});
                        writer.send(Message::Text(close_msg.to_string().into()))
                            .await
                            .map_err(|e| format!("close stream failed: {}", e))?;
                        finish_sent = true;
                        emit_status(&app, PROVIDER_DEEPGRAM, "finishing");
                    }
                }
            }
            message = reader.next() => {
                match message {
                    Some(Ok(Message::Text(text))) => {
                        if handle_message(&app, &text) {
                            break;
                        }
                    }
                    Some(Ok(Message::Close(_))) => break,
                    Some(Ok(_)) => {}
                    Some(Err(e)) => return Err(format!("ws read error: {}", e)),
                    None => break,
                }
            }
            else => break,
        }
    }

    mark_runtime_finished();
    emit_status(&app, PROVIDER_DEEPGRAM, "closed");
    Ok(())
}

/// Handle a Deepgram JSON message. Returns true if session should end.
fn handle_message(app: &AppHandle, text: &str) -> bool {
    let payload: Value = match serde_json::from_str(text) {
        Ok(v) => v,
        Err(_) => return false,
    };

    let msg_type = payload.get("type").and_then(Value::as_str).unwrap_or_default();

    match msg_type {
        "Results" => {
            let is_final = payload
                .get("is_final")
                .and_then(Value::as_bool)
                .unwrap_or(false);

            let transcript = payload
                .get("channel")
                .and_then(|c| c.get("alternatives"))
                .and_then(Value::as_array)
                .and_then(|alts| alts.first())
                .and_then(|alt| alt.get("transcript"))
                .and_then(Value::as_str)
                .unwrap_or_default()
                .trim()
                .to_string();

            if transcript.is_empty() {
                return false;
            }

            if is_final {
                push_final_transcript(&transcript);
            } else {
                update_partial_transcript(&transcript);
            }
            emit_transcript(app, &transcript, is_final);
        }
        "Metadata" => {
            // Connection metadata, ignore
        }
        "Error" => {
            let msg = payload
                .get("message")
                .and_then(Value::as_str)
                .unwrap_or("unknown error");
            mark_runtime_error(msg);
            emit_status(app, PROVIDER_DEEPGRAM, &format!("error: {}", msg));
            return true;
        }
        _ => {}
    }

    false
}

/// Test Deepgram connection by connecting and waiting for metadata.
pub async fn test_connection(api_key: &str, endpoint: &str, model: &str) -> Result<String, String> {
    if api_key.is_empty() {
        return Err("API key is required".to_string());
    }

    let url = build_url(endpoint, model, "");
    let mut request = url
        .into_client_request()
        .map_err(|e| format!("invalid endpoint: {}", e))?;

    request.headers_mut().insert(
        "Authorization",
        format!("Token {}", api_key)
            .parse()
            .map_err(|e| format!("invalid auth header: {}", e))?,
    );

    let (socket, _) = connect_async(request)
        .await
        .map_err(|e| format!("Deepgram connect failed: {}", e))?;
    let (mut writer, mut reader) = socket.split();

    // Wait briefly for metadata or error
    let response = tokio::time::timeout(
        tokio::time::Duration::from_secs(5),
        reader.next(),
    )
    .await
    .map_err(|_| "Timed out waiting for Deepgram response".to_string())?;

    match response {
        Some(Ok(Message::Text(text))) => {
            let payload: Value = serde_json::from_str(&text)
                .map_err(|e| format!("invalid response: {}", e))?;
            let msg_type = payload.get("type").and_then(Value::as_str).unwrap_or_default();
            if msg_type == "Error" {
                let msg = payload.get("message").and_then(Value::as_str).unwrap_or("auth failed");
                return Err(msg.to_string());
            }
            // Send close
            let close_msg = serde_json::json!({"type": "CloseStream"});
            let _ = writer.send(Message::Text(close_msg.to_string().into())).await;
            Ok("Deepgram connection test succeeded".to_string())
        }
        Some(Ok(Message::Close(frame))) => {
            let reason = frame.map(|f| f.reason.to_string()).unwrap_or_default();
            Err(format!("Connection closed: {}", reason))
        }
        Some(Ok(_)) => Ok("Deepgram connection test succeeded".to_string()),
        Some(Err(e)) => Err(format!("ws error: {}", e)),
        None => Err("Connection closed immediately".to_string()),
    }
}

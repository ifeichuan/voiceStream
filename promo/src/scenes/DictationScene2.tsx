...
export const DictationScene2: React.FC = () => {
  ...
  const rawText = '就是说我们这个 API 的话它的 response time 太长了你知道吗平均要两秒多这个肯定是不行的';
  const cleanText = '我们这个 API 的 response time 太长了，平均要两秒多，这个肯定是不行的';

  const rawChars = Math.floor(interpolate(frame, [20, 65], [0, rawText.length], {extrapolateRight: 'clamp'}));
  const showClean = frame > 75;
  const cleanOpacity = interpolate(frame, [75, 88], [0, 1], {extrapolateRight: 'clamp'});
  const rawStrike = interpolate(frame, [72, 80], [0, 1], {extrapolateRight: 'clamp'});
  ...
  return (
    <AbsoluteFill style={{
      ...
      <div style={{
        ...
        <RemotionShaderOrb
          size={140}
          isActive={frame > 10 && frame < 70}
          audioLevel={frame < 70
            ? interpolate(frame, [10, 25, 40, 60], [0, 0.6, 0.8, 0.5], {extrapolateRight: 'clamp'})
            : 0
          }
        />
      </div>
